...
void main() {
    ...
    float diffuselight = 0.9 * clamp(dot(fs_in.normal,lightDir),0.35,1.0);


    float specularLight = reflectivity * pow(max(dot(reflect(lightDir, fs_in.normal), normalize(fs_in.position - pos)),0),shineDamper);
